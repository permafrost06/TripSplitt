...
export function GroupExpenseForm({
    ...
    const [totalAmount, setTotalAmount] = useState(initialExpense?.amount?.toString() || '');
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (Math.abs(itemsTotal - parsedAmount) > 0.01) {
            setError(
                `Items total (${itemsTotal.toFixed(2)}) doesn't match total amount (${parsedAmount.toFixed(2)})`
            );
            return;
        }
        ...
        <Card>
            <CardHeader>
                <div className="flex items-center justify-between">
                    <CardTitle>
                        {initialExpense ? 'Edit Expense Group' : 'Add Expense Group'}
                    </CardTitle>
            ...
            <CardContent>
                <form onSubmit={handleSubmit} className="space-y-5">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="space-y-2">
                            ...
                            <Input
                                ...
                                placeholder="Expense group description"
                                autoFocus
                            />
                        </div>
                        <div className="space-y-2">
                            <Label htmlFor="expenseTotalAmount">Total Amount</Label>
                    ...
                    <div className="border-t border-stone-200 dark:border-stone-800 pt-5">
                        <div className="sticky top-[72px] z-10 bg-white dark:bg-stone-950 pt-2 pb-2 -mx-4 px-4 mb-4">
                            <Button
                                type="button"
                                variant="outline"
                                onClick={addItem}
                        ...
                        {items.length === 0 ? (
                            <p className="text-sm text-stone-500 dark:text-stone-400 text-center py-4">
                                No items added yet. Click "Add Item" to add items to this expense
                                group.
                            </p>
                        ) : (
                    ...
                    <div className="flex gap-3">
                        <Button type="submit" className="flex-1">
                            {initialExpense ? 'Save Changes' : 'Add Expense Group'}
                        </Button>
