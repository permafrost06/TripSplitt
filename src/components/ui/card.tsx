...
const Card = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
    ({ className, ...props }, ref) => (
        <div
            ref={ref}
            className={cn(
                'border border-stone-200 dark:border-stone-800 bg-white dark:bg-stone-950 transition-all',
                className
            )}
            {...props}
        />
...
const CardTitle = React.forwardRef<HTMLParagraphElement, React.HTMLAttributes<HTMLHeadingElement>>(
    ({ className, ...props }, ref) => (
        <h3
            ref={ref}
            className={cn(
                'text-xl font-serif leading-tight text-stone-900 dark:text-stone-100',
                className
            )}
            {...props}
        />
